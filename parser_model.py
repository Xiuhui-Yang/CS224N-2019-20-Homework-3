...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.tensor(np.zeros((self.embed_size*self.n_features,self.hidden_size))).float())
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        self.embed_to_hidden_bias = nn.Parameter(torch.tensor(np.zeros((self.hidden_size))).float())
        nn.init.uniform_(self.embed_to_hidden_bias)
        self.dropout = nn.Dropout(p=dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(torch.tensor(np.zeros((self.hidden_size,self.n_classes))).float())
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)
        self.hidden_to_logits_bias = nn.Parameter(torch.tensor(np.zeros(( self.n_classes))).float())
    ...
    def forward(self, w):
        ...
        x = F.relu(x)
        logits = x.mm(self.hidden_to_logits_weight) + self.hidden_to_logits_bias
